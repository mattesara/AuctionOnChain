...
def auction_detail(request, pk):
    ...
    if now >= auction.end_time and not auction.is_closed:
    ...
    bid_form = BidForm(request.POST or None, instance=Bid(item=auction))
    ...
    if request.method == 'POST':
        if bid_form.is_valid():
            if not auction.is_closed:
                ...
                redis_client.zadd(cache_key, {serialized_data: time.mktime(bid.datetime.timetuple())})
                latest_bid_serialized = redis_client.zrevrange(cache_key, 0, 0, withscores=True)
                latest_bid = json.loads(latest_bid_serialized[0][0].decode('utf-8'))
    ...
    time_remaining = max(auction.end_time - now, timezone.timedelta())

    transaction = Transaction.objects.filter(auction=auction).last()
    last_bids_serialized = redis_client.zrevrange(cache_key, 0, 2)
    last_bids = []

    for last_bid_serialized in last_bids_serialized:
